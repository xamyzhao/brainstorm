...
if __name__ == '__main__':
	...
	for ei, exp_type in enumerate(args.exp_type):
		if exp_type.lower() == 'trans':
			'''''''''''''''''''''''''''
			Transform (spatial or appearance) learner
			'''''''''''''''''''''''''''
			...
			if args.aug_vte:
				...
				'''
				data_params['vte_flow_model'] = args.aug_vte[0]
				if len(args.aug_vte) > 1:
					data_params['vte_color_model'] = args.aug_vte[1]
				else:
					data_params['vte_color_model'] = None
				'''
				data_params['load_vols'] = False
